...
if (CSSA3_selectedTab == "Players") exitWith {
...

	{
		if ((alive _x)&&{(isPlayer _x)}) then {
			_colour = call {
				if (side group _x == WEST) exitWith {_unitColour = [0.098,0.361,0.62,1]};
				if (side group _x == EAST) exitWith {_unitColour = [0.502,0,0,1]};
				if (side group _x == civilian) exitWith {_unitColour = [0.373,0.016,0.706,0.7]};
				if (side group _x == resistance) exitWith {_unitColour = [0,1,0,0.7]};
				_unitColour = [0.251,0.251,0.251,1];
		    };
