...
if (CSSA3_visibleHUD) then {
	...
	if (CSSA3_settings select 1 == 1) then {_iconSize = [0.8,0.8]};
	{
		if ((alive _x) && {_cam distance _x < _fadeEnd}) then {
			...
			_color = call {
				if (side group _x == east) exitWith {_iconColour = [1,0,0,0.6]};
				if (side group _x == west) exitWith {_iconColour = [0,0,1,0.6]};
				if (side group _x == civilian) exitWith {_iconColour = [0.373,0.016,0.706,0.6]};
				if (side group _x == resistance) exitWith {_iconColour= [0,1,0,0.6]};
			};
			...
			_icon = ["\A3\ui_f\data\IGUI\RscIngameUI\RscUnitInfoAirRTDFull\whitedot_ca.paa",_iconColour,_pos,_iconSize select 0,_iconSize select 1,0,_nameText,1, ((0.16 / (_distance max 0.01)) * 2) min 0.08];
