	...
	while{(call rhs_fnc_findPlayer)==gunner _v OR (call rhs_fnc_findPlayer)==driver _v}do
	{
		if(_f!=(parseNumber (ctrlText _z)))then
		{
			if((call rhs_fnc_findPlayer)==gunner _v)then{
				if(parseNumber (ctrlText _z)==12 AND _v animationPhase "sightHide" !=0)then
				{
					_v animate ["sightHide",0,true];_c ctrlSetText "\rhsafrf\addons\rhs_t72\data\RF\TPD_K1_lasermark_6x.paa";
				}else{
					if(_v animationPhase "sightHide" ==0 AND parseNumber (ctrlText _z)!=12)then{_v animate ["sightHide",0.6,true];_c ctrlSetText "";};
				};
			};
			_f=parseNumber (ctrlText _z);
		};
