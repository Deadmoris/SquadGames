...
if(isNull (uiNamespace getVariable ["RHS_1PZ32_Ctrl",displayNull]))then
{
	...
	[] spawn {
		...
		while{not(isNull _c)}do
		{
			...
			_p=(call rhs_fnc_findPlayer);
			_currentZoom = round (([0.5,0.5] distance worldToScreen positionCameraToWorld [0,1.05,1]) *(getResolution select 5)* 10) / 10;
			if((currentVisionMode _p == 1) OR (_currentZoom == 1.5))then{
				if(_s)then{
