...
[{
    ...
    if (!EGVAR(interact_menu,keyDown)) then {
        ...
        if ((!isNull cursorTarget) && {cursorTarget isKindOf "Static"} && {!(cursorTarget in _housesScaned)}) then {
            if (((count (configFile >> "CfgVehicles" >> (typeOf cursorTarget) >> "UserActions")) > 0) 
                || {(count (getArray (configFile >> "CfgVehicles" >> (typeOf cursorTarget) >> "ladders"))) > 0}
                || { count (getarray (configFile >> "CfgVehicles" >> (typeOf cursorTarget) >> "AnimationSources")) > 0 } ) then {
        ...
        if (_housesToScanForActions isEqualTo []) then {
            ...
            private _nearBuidlings = nearestObjects [ace_player, ["Static"], 30];
            {
                ...
                if (((count (configFile >> "CfgVehicles" >> _typeOfHouse >> "UserActions")) == 0) 
                    && {(count (getArray (configFile >> "CfgVehicles" >> _typeOfHouse >> "ladders"))) == 0}
                    && { count (getarray (configFile >> "CfgVehicles" >> (typeOf cursorTarget) >> "AnimationSources")) > 0 }) then {
