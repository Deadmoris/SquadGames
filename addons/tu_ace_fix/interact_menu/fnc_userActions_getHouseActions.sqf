 ...
 * Public: No
 */
#include "\z\ace\addons\interact_menu\functions\script_component.hpp"
...
private _fnc_hitpoint_conditional = {
    params ["_target", "_player", "_variable"];
    _variable params ["_hitpoint"];
    private _building = _target getVariable [QGVAR(building), objNull];
    _building getHitPointDamage _hitpoint < 1
};

private _fnc_hitpoint_hit = {
    params ["_target", "_player", "_variable"];
    _variable params ["_hitpoint", "_selection"];
    private _building = _target getVariable [QGVAR(building), objNull];
    private _sound  = selectRandomWeighted (getarray (configfile >> "CfgAmmo" >> "BulletBase" >> "hitGlass"));    
    if( !isNil { _sound } ) then {
        _sound = getarray (configfile >> "CfgAmmo" >> "BulletBase" >> _sound);
        private _file = _sound#0; 
        private _parts = _file splitString "\."; 
        if(_parts select (count _parts - 1) != "ogg" ) then { _file = _file + ".wss" };

        playsound3d [_file, _building, false, _building modelToWorldWorld  (_building selectionPosition _selection), _sound#1, _sound#2, _sound#3 ];
    };
    [_building, [_hitpoint, 1]] remoteExec ["setHitPointDamage", 0];
};

_configPath = configFile >> "CfgVehicles" >> _typeOfBuilding >> "AnimationSources";
for "_index" from 0 to ((count _configPath) - 1) do {
    private _animationSource = _configPath select _index;
    if( getText (_animationSource >> "source") == "Hit" ) then {
        private _hpName = getText (_animationSource >> "hitpoint");
        private _hp = configFile >> "CfgVehicles" >> _typeOfBuilding >> "HitPoints" >> _hpName;
        private _actionPosition = getText (_hp >> "name");
        private _actionOffset = [_actionPosition] call _fnc_getMemPointOffset;
        private _memPointIndex = _memPoints find _actionPosition;
        private _action = [format ["Hit_%1", _hpName], localize "STR_TU_Break", "", _fnc_hitpoint_hit , _fnc_hitpoint_conditional, {}, [_hpName, _actionPosition], _actionPosition, 2, [false,false,false,false,true]] call EFUNC(interact_menu,createAction);
        (_memPointsActions select _memPointIndex) pushBack _action;
    };
};
