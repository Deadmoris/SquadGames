...
#include "\z\ace\addons\frag\functions\script_component.hpp"
...
private _fragRange = 4 * _indirectHitRange * 4;
...
if (!(_objects isEqualTo [])) then {
    ...
    };
    {
        ...
        if (alive _target && (_indirectHitRange < _distance) || {random 1 > ((_distance-_indirectHitRange)/(_fragRange-_indirectHitRange))^2 }) then {
