#define ADDON tu_ace_fix
#define FUNC(x) ADDON##_fnc_##x
#define FUNC2(y,x) y##_fnc_##x
...
#define ACEOVERRIDE(addon,preStart) \
class ACE(addon) { \
    init = Q(CALL2(addon,preStart)); \
};

#define ACEOVERRIDEFUNC(addon,fnc,file) [PATH2(addon,file),QACEFUNC(addon,fnc)] call CBA_fnc_compileFunction
