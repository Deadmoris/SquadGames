...
if (CBA_missionTime < GVAR(nextEffectsUpdate)) then {
    ...
    private _lightFinal = 2 min (GVAR(NvgMinLightIntensity) max (GVAR(NvgEffectiveLightMultiplier) + moonIntensity - ((overcast * .8) min .275) - (rain * .5)));
    private _effectiveLight = _lightFinal * linearConversion [1, 3, GVAR(nvgGeneration), 1, 2];
    private _effectMod = linearConversion [1, 3, GVAR(nvgGeneration), 1.5, 1] * GVAR(NvgEffectModMultiplier);
