...
// AleM's way to separate NVDs PP ColorCorrection Params (JDT)
private _offset = 0;
...
if (alive ACE_player) then {
    if (((vehicle ACE_player) == ACE_player) || {
        ...
        if ((cameraView == "GUNNER") && {currentWeapon ACE_player != ""} && {binocular ACE_player == currentWeapon ACE_player}) exitWith {
            ...
            // Gets right Color Correction Preset from CfgWeapons (JDT)
        ...
        if (isNumber (_config >> QGVAR(generation))) then {_nvgGen = getNumber (_config >> QGVAR(generation));};
        // Gets right Color Correction Preset from CfgWeapons.hpp (JDT)
        if (isArray (_config >> "colorPreset")) then {_preset = getArray (_config >> "colorPreset");};
    		_offset = _preset select 0;
    		_blend = _preset select 1;
    		_colorize = _preset select 2;
...
// Additional for colorPreset supporting (JDT)
