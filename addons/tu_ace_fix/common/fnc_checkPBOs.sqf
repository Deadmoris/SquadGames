...
if (!isServer) then {
    [{
        ...
        ACE_Version_ClientErrors params ["_missingAddon", "_missingAddonServer", "_oldVersionClient", "_oldVersionServer","_missingAddons","_missingAddonsServer"];
        ...
        if (_missingAddon || {_missingAddonServer} || {_oldVersionClient} || {_oldVersionServer}) then {
            ...
            if (_missingAddon) then {
                _text = _text + "Detected missing addon on client<br/>" + str _missingAddons;
                _error = _error + "Missing: " + str _missingAddon;
            };
            if (_missingAddonServer) then {
                _text = _text + "Detected missing addon on server<br/>" + str _missingAddonsServer;
                _error = _error + "Additional file(s); ";
            };
            if (_oldVersionClient) then {
                _text = _text + "Detected old client version<br/>"+ str _missingAddon;
                _error = _error + "Older version; ";
            };
            if (_oldVersionServer) then {
                _text = _text + "Detected old server version<br/>"+ str _missingAddon;
