...
_server = (isdedicated or (if (!isnil 'headlessclient') then {local headlessclient} else {false}));
diag_log ('priv ' + str _server);
if (!_server && (isNil "tu_arty_debug_mode")) exitWith {};
