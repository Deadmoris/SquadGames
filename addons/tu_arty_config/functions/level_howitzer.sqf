...
if ((_gun_pos select 2)>1) exitWith {  
	hint "Здесь нельзя горизонтировать";  
};
...
private _zmax = getTerrainHeightASL _gun_pos;  
private _zmin = _zmax;  
...
} forEach [_left_back, _right_back, _front_center]; //forEach [_left_back select 2, _right_back  select 2, _front_center  select 2]; 
...
[_gun, _zmax-_zmin, _delta_z] spawn {
	...
	[_duration, [_gun, _zmax], {  
		...
		player setPosATL [getPos player select 0, getPos player select 1,(_foundation_position select 2)+BN_FOUNDATION_HEIGHT+0.2]; 
		["BN_LEVEL_GUN_TOKEN", player, _gun] remoteExec ["tu_arty_fnc_give_up_token",2];
	}, {private _gun = _this select 0 select 0; hint "Горизонтирование прервано!"; ["BN_LEVEL_GUN_TOKEN", player, _gun] remoteExec ["tu_arty_fnc_give_up_token",2];}, "Горизонтирование орудия"] call ace_common_fnc_progressBar;
};