...
params ["_angleDeg", "_muzzleVelocity", "_airFriction", "_temp", "_relDensity", "_tailWind", "_crosswind", "_heightOfTarget", "_timeStep","_derivation"];
...
while {((_currentVelocity select 2) > 0) || ((_currentPos select 2) >= _heightOfTarget)} do {
	...
	private _dderivation = _derivation;	
	private _derivationDir = [0 - (_currentVelocity select 1),_currentVelocity select 0,0];	
	_derivationDir = vectorNormalized _derivationDir;
	_derivationDir = _derivationDir vectorMultiply _dderivation;
    _currentVelocity = _currentVelocity vectorAdd (_derivationDir vectorMultiply _timeStep);	
