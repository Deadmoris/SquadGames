...
* 3) since Nov 1, 2018: взрыв дымовых мин
* 
...
if (isNil 'tu_arty_observable_list' || {(count tu_arty_observable_list == 0) && !(_shell in tu_arty_observable_list)}) then {
	...
	tu_arty_observable_positions = [getPosASL _shell]; 
	...
	["tu_arty_monitor", "onEachFrame", {
		for "_i" from (count tu_arty_observable_list) to 0 step -1 do { // нумерация вагонов с хвоста поезда для эффективного удаления из массива
			...
			if (!(isNull _shell) && {alive _shell} && {(getPos _shell select 0)>1}) then {
				...
				tu_arty_observable_positions set [_i,getPosASL _shell];
			} else {
				// impact
				_place = (tu_arty_observable_positions select _i);
				[tu_arty_observable_types select _i, _place] call tu_arty_fnc_smoke_burst;
				private _color = (tu_arty_observable_authors select _i) getVariable ["tu_arty_explosion_marker_color", "ColorBlack"];
				_object = if (isnil 'headlessclient') then {2} else {headlessclient};
				[_place select 0, _place select 1, tu_arty_observable_types select _i, _color] remoteExec ["tu_arty_fnc_explosion_marker", _object, false];
			...
			private _typeOfShell = typeOf _shell;
			//hint _typeOfShell;
			if (((_typeOfShell == "bn_60mm_frag_prx") || (_typeOfShell == "bn_81mm_frag_prx")) && {((velocity _shell) select 2) < -1} && {(getPosATL _shell select 2) < 5}) then { // TODO: бить себя по рукам за хардкод класснеймов в скриптах
				[_shell, tu_arty_observable_authors select _i, _typeOfShell] call tu_arty_fnc_proximity_burst;
			};
		};
