...
while {
	((player getVariable ['tu_arty_prepare_status',2]) >= 1) && {_n>0} && {((player distance _holder) < 2) && (_holder != (backpackContainer player))} && {(alive player) && (alive _source) && (simulationEnabled player)};
	//tu_arty_prepare_status: 2 - busy, 1 - ready, 0 - cancel
		
} do {
	[_prep_time,
		[_classname_prepped, _holder, _n],{
			...
			if ((_tu_arty_params select 2) > 1) then {
					hint format ['Осталось %1', (_tu_arty_params select 2) - 1];	
				} else {
				hint 'Готово.';
			};	
			...
			player setVariable ['tu_arty_prepare_status',0];
	...
	waitUntil {
		sleep 0.1;
		(player getVariable ['tu_arty_prepare_status',2]) < 2
	};
	
	if ((player getVariable ['tu_arty_prepare_status',0]) == 0) exitWith {
		_source addMagazineCargoGlobal [_classname_unprepped, _n];
		_n = 0;
	};
	
	if ((((player distance _holder) < 2) && (_holder != (backpackContainer player))) && ((alive player) && (alive _source))) then {
		_n = _n - 1;
	};
...
if (count (weaponCargo _holder - ['FakeWeapon']) == 0) then {
