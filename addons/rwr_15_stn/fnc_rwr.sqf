...
//INIT
private ["_type","_threats","_lastScan","_RWRopen","_MaxTargets","_scanInterval","_oldThreats"];
_type = 0;
_threats = [];
_lastScan = [];
_oldThreats = [];
...
while { (_crew == driver _vehicle || {_crew == gunner _vehicle} || {_crew == commander _vehicle}) && {alive _crew} && {alive _vehicle} && (uiNamespace getVariable "ACE_RWR_STATUS")} do {
		...
		if (count (_threats - _invisibleThreats) > 0) then {
			...
			for "_y" from 0 to (count _threats - 1) do {
				...
				if (_threat in _threats && {!(_threat in _invisibleThreats)}) then {
					...
					if(_threat isKindOf "tu_pancir_sa_22")then{
						_type_index = 3;
					};
	...
	for "_y" from 0 to (count _threats - 1) do {
		...
		if (_threat distance _vehicle > (getNumber (configFile >> "cfgVehicles" >> typeOf _threat >> "rangerwrnew"))) then { //THREAT OUT OF RANGE
			_rmthreats set [count _rmthreats, _threat];
		};
	};
