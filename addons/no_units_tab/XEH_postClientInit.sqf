...
_addGroupUnitToDiary = 
{
	...
	_text = _text + "<font color='#FFFFBB'>" + (str _number) + ". " + (name _unit) + "</font>" + (if(leader _unit == _unit)then{" (Командир)"}else{""});
        _text = _text + "<br/>";
	if(primaryWeapon _unit != "")then
	{
		_name = getText(configFile >> "CfgWeapons" >> (primaryWeapon _unit) >> "displayName");
		_text = _text + "Основное оружие - " + _name;
                _text = _text + "<br/>";
	};
...
player createDiaryRecord ["diary", ["Мой отряд (" + _name + ")", _text]];
