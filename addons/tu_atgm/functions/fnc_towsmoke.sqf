...
if (__towsmoke == 1) then
{
	...
	while {alive _mis} do
	{
		...
		{
			drop ["\A3\Data_f\kouleSvetlo","","Billboard",100,0.1,_mis modelToWorld (_x select 0),(_x select 1),0,_wei,_vol,0.05,[0.9],[[1,1,0.3,0.8]],[0],0,0,"","",""];
		...
		{	//while sustain motor works and the missile is visible
			...
			{
				drop ["\A3\Data_f\missileSmoke","","Billboard",100,4+random 2, _mis modelToWorld (_x select 0),(_x select 1),0,_wei,_vol,0.5,[1,2,3,4],[[0.35,0.35,0.35,0.3],[0.25,0.25,0.25,0.2],[0.2,0.2,0.2,0.1]],[0],0,0,"","",""];
...
else
{
	...
	{
		drop ["\A3\Data_f\kouleSvetlo","","Billboard",100,0.03,_mis modelToWorld [0,0,0],[0,0,0],0,_wei,_vol,0.05,[0.5],[[1,1,0.3,0.7]],[0],0,0,"","",""];
		sleep 0.005;
	};
};