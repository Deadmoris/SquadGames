...
private ["_vehicle","_vType", "_Display_HeadingText", "_Display_RangeText", "_Display_LaunchAuthorization","_Display_LaunchAuthorization2","_lastTime","_sleepTime","_Display_Cross","_Display_Range","_Display_LaunchAuthorization3","_Display_ka52"];

params["_Generic_Display"];
...
_Display_LaunchAuthorization2 =	_Generic_Display displayCtrl 694004;
_Display_ka52 = _Generic_Display displayCtrl 694008;

// Pilot's
_Display_Cross =	_Generic_Display displayCtrl 694005;
_Display_Range =	_Generic_Display displayCtrl 694006;
_Display_LaunchAuthorization3 =	_Generic_Display displayCtrl 694007;
...
_Display_ka52 ctrlShow false;

// Pilot's stuff
_Display_Cross ctrlShow false;
_Display_Range ctrlShow false;
_Display_LaunchAuthorization3 ctrlShow false;
...
_Display_LaunchAuthorization3 ctrlsetposition [(_p select 0),(_p select 1)-0.04*SafezoneH,(_p select 2)/_k,0.03*SafezoneH/_k];
_Display_LaunchAuthorization3 ctrlsetscale _k;
_Display_LaunchAuthorization3 ctrlcommit 0;
...
_tracker = getText (configFile >> "CfgVehicles" >> _vType >> "Turrets" >> "MainTurret" >>"tu_atgm_tracker");
_lastTime = time;
_sleepTime = 0.1;

_hudSize = if (_vehicle isKindOf "RHS_Mi24_base") then {4} else {8};	// Half angular size
_hudArea = 0.4;	// Maximum screen offset when FoV = 0.7
_gunnerHudArea = if (_vehicle isKindOf "RHS_Mi24_base") then {0.1} else {0};	// Maximum screen offset when FoV = 0.7
...
_Display_LaunchAuthorization3 ctrlsettextcolor _color;
...
while {player in [gunner _vehicle, driver _vehicle] && {alive _vehicle}} do {
	...
	//	_hdng = getDir _vehicle;
		_wdir = ([_vehicle modelToWorld (_vehicle selectionPosition (getText (__MainTurret >> "gunBeg"))), _vehicle modelToWorld (_vehicle selectionPosition (getText (__MainTurret >> "gunEnd")))] call BIS_fnc_vectorDiff) call tu_atgm_fnc_mat_normalize3d;
		...
		// Format Range Text
		_range = _vehicle getVariable ["RU_DISTANCE_TO_TARGET", 0];
		...
		if ((player == gunner _vehicle) && {_vehicle isKindOf "RHS_Mi24_base"} or {_vehicle isKindOf "rhsgref_mi24g_base"}) then {
			if ((([configFile >> "CfgWeapons" >> (currentWeapon _vehicle) , configFile >> "CfgWeapons" >> "MissileLauncher"] call CBA_fnc_inheritsFrom) && (isNil {_vehicle getVariable "tu_atgm_exceeded"})) && (isNil {_vehicle getVariable "RU_LAUNCH_AUTHORIZED"})) then {
		...
		_sleepTime = 0.1;
		// Launch authorization Mi-28
		if ((player == gunner _vehicle) && {_vehicle isKindOf "rhs_mi28_base"}) then {
			if ((([configFile >> "CfgWeapons" >> (currentWeapon _vehicle) , configFile >> "CfgWeapons" >> "MissileLauncher"] call CBA_fnc_inheritsFrom) && (isNil {_vehicle getVariable "tu_atgm_exceeded"})) && (isNil {_vehicle getVariable "RU_LAUNCH_AUTHORIZED"})) then {
		...
		// Launch authorization Ka-52
		if ((player == gunner _vehicle) && {_vehicle isKindOf "RHS_Ka52_base"}) then {
			if ((([configFile >> "CfgWeapons" >> (currentWeapon _vehicle) , configFile >> "CfgWeapons" >> "MissileLauncher"] call CBA_fnc_inheritsFrom)) && (isNil {_vehicle getVariable "RU_LAUNCH_AUTHORIZED"})) then {
				_Display_ka52 ctrlShow true;
				_vehicle setVariable ["RU_LAUNCH_AUTHORIZED", true];
			} else {
				if !((([configFile >> "CfgWeapons" >> (currentWeapon _vehicle) , configFile >> "CfgWeapons" >> "MissileLauncher"] call CBA_fnc_inheritsFrom)) || (isNil {_vehicle getVariable "RU_LAUNCH_AUTHORIZED"})) then {
					_Display_ka52 ctrlShow false;
		...
		if !(isNil "RU_DISPLAYING_OPTICS") then {
			...
			_Display_LaunchAuthorization2 ctrlShow false;	// Launch authorization Mi-28
			_Display_ka52 ctrlShow false;	// Deactivate ka52 launch authorization.
	...
	if (isNil {_vehicle getVariable "tu_atgm_laser_designation"}) then {
		...
		if (isNull _pulse) then {_pulse = (_vehicle getVariable "tu_atgm_opticalTarget")};
	...
	[_vehicle,_vehicle getVariable "RU_DISTANCE_TO_TARGET",magazines _vehicle,weapons _vehicle,true] call tu_atgm_fnc_setsolution;
	...
	if (cameraView == "INTERNAL" && {!visibleMap} && {isEngineOn _vehicle} && {player in [driver _vehicle,gunner _vehicle]}) then {
		if (player == driver _vehicle) then {
			if (isNil "RU_DISPLAYING_HUD") then {
				...
				_Display_LaunchAuthorization3 ctrlShow true;
				***************************/
				_Display_Cross ctrlsettext "\tu_atgm\data\crossdot_ca.paa";
				_Display_Cross ctrlSetTextColor _color;
			...
			_avec = [_wdir,2000] call tu_atgm_fnc_vectorMultiply;
			_apos = [_vpos,_avec] call tu_atgm_fnc_vectorAdd;
			...
			if (count _apos == 2 && {(abs(acos([_vdir,_wdir] call tu_atgm_fnc_dotProduct)) < _hudSize)}) then {
				...
				if (sqrt((0.5-(_apos select 0))^2+(0.5-(_apos select 1))^2) < _hudArea*(0.7/_fov)) then {
					...
					if ([configFile >> "CfgWeapons" >> (currentWeapon _vehicle) , configFile >> "CfgWeapons" >> "MissileLauncher"] call CBA_fnc_inheritsFrom) then {
						if (isNil {_vehicle getVariable "tu_atgm_exceeded"}) then {
							_Display_LaunchAuthorization3 ctrlsetposition [(_apos select 0)-_xoff*_scale,(_apos select 1)+ _yoff*_scale -0.01*SafezoneH*_scale ];
							_Display_LaunchAuthorization3 ctrlsetscale _scale*_k;
							_Display_LaunchAuthorization3 ctrlcommit 0;
						} else {
							_Display_LaunchAuthorization3 ctrlsetposition [-10,-10];
							_Display_LaunchAuthorization3 ctrlcommit 0;
						};
						_Display_Cross ctrlsettext "\tu_atgm\data\cursor_w_grenade_gs.paa";
					} else {
						_Display_Cross ctrlsettext "\tu_atgm\data\crossdot_ca.paa";
						_Display_LaunchAuthorization3 ctrlsetposition [-10,-10];
						_Display_LaunchAuthorization3 ctrlcommit 0;
			...
			if (isNil "RU_DISPLAYING_HUD") then {
			    /***************************
				_Display_Cross ctrlShow true;
				_Display_Cross ctrlsettext "\tu_atgm\data\ks53_ca.paa";
		...
		if !(isNil "RU_DISPLAYING_HUD") then {
			...
			_Display_LaunchAuthorization3 ctrlShow false;	// Launch authorization
...
if (_vehicle getVariable "RU_SYSTEMS_LOADED") then {
    ...
    _Display_LaunchAuthorization2 ctrlShow false;	// Deactivate Mi-28 launch authorization.
	_Display_ka52 ctrlShow false;	// Deactivate ka-52 launch authorization.
