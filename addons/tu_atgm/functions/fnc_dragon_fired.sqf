...
if ((_this select 0) == player || {(gunner (_this select 0)) == player}) then {
	if ((typeOf (_this select 6)) == "rhs_ammo_9m14m") then {
		...
		_newVel = [_unitVec, DRAGON_VELOCITY*1.25] call tu_atgm_fnc_vectorMultiply;
		...
		[
			{
				_unitVec = (velocity _shell) call tu_atgm_fnc_unitVector;
				...
				if (count _screenPos > 0 && {alive _gunner}) then {
					...
					if (diag_tickTime >= _timerCorrect && {_chargeCount > 0}) then {
						if ((abs _dir) >= _xDeg/2) then {
							...
							_shell setVelocity _newVel;
							_shell setVectorDir  (_newVel call tu_atgm_fnc_unitVector);
							/**************
			...
			},
			{
				if !(isNull _missile) then {
					...
					_missile setVectorDir (_newVel call tu_atgm_fnc_unitVector);
