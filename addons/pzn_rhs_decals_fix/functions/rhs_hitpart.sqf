/*
	...
	Example:	_this call RHS_fnc_hitPart
*/
/* 
// Exit if AIS is running
if !(isNil "AIS_PENETRATION_ENABLED") exitWith {};
...
/*
private "_b";
_b = createVehicle ["Sign_Sphere10cm_F", _pos, [], 0, "CAN_COLLIDE"];
_b setObjectTexture [0, '#(argb,8,8,3)color(0.0,0.0,0.5,0.1,ca)'];
systemChat str [time, _pen select 0, _vec distance [0,0,0], diag_tickTime, diag_frameno];
*/
/* 
...
if (_count == 1) then {
	...
	[_pen,_pos,_vec,_count,_rev,_anm,_v] spawn {
		...
		for "_i" from 1 to (_this select 3)-1 do {
			...
			private "_b";
			_b = createVehicle ["Sign_Sphere10cm_F", _pos, [], 0, "CAN_COLLIDE"];
			_b setObjectTexture [0, '#(argb,8,8,3)color(0.5,0.0,0.0,0.1,ca)'];
			systemChat str [time, _pen select _i, _vec distance [0,0,0], diag_tickTime, diag_frameno];
			*/
		/* };
